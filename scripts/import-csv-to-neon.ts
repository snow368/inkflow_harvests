...
async function main() {
    ...
    const promises = batch.map(async (row, batchIdx) => {
      ...
      try {
        ...
        const address = [
          ...
          clean(row['Default Address Province Code'] || ''),
          clean(row['Default Address Country Code'] || '')
        ].filter(Boolean).join(', ');
        ...
        const matchedArtist = email ? emailIndex.get(email) :
                              phone ? phoneIndex.get(phone) :
                              fullName.toLowerCase() !== 'shopify customer' ? nameIndex.get(fullName.toLowerCase()) : undefined;

        if (matchedArtist) {
          const newOrderCount = (matchedArtist.order_count || 0) + totalOrders;
          const newTotalSpent = (matchedArtist.total_spent || 0) + totalSpent;
          const newCustomerTier = newOrderCount >= 2 ? 'loyal' : 'new';

          await sql`
            UPDATE artists SET
              order_count = ${newOrderCount},
              total_spent = ${newTotalSpent},
              total_items = ${(matchedArtist.total_items || 0) + totalItems},
              last_order_date = NOW(),
              stage = 'customers',
              customer_tier = ${newCustomerTier},
              dna_tags = ARRAY(SELECT DISTINCT unnest(dna_tags || ARRAY['#ShopifyCustomer', '#Purchased'])),
              last_updated = NOW()
            WHERE id = ${matchedArtist.id}
          `;
          updatedCount++;
        } else {
          const id = `shopify_${index}_${Date.now()}`.replace(/\//g, '_');
          const username = email.split('@')[0] || `shopify_user_${index}`;

          await sql`
            INSERT INTO artists (id, uid, username, full_name, shop_name, stage, heat_score, similarity_score, dna_tags, followers, order_count, total_spent, total_items, location, city, state, country, last_order_date, customer_tier, account_tag, last_updated, metadata)
            VALUES (
              ${id}, ${UID}, ${username}, ${fullName}, ${fullName}, 'customers', 50, 0,
              ARRAY['#ShopifyCustomer', '#Purchased'], 0, ${totalOrders}, ${totalSpent}, ${totalItems},
              ${importRegion || city || 'Unknown'}, ${city || null}, ${null}, ${country},
              NOW(), ${totalOrders >= 2 ? 'loyal' : 'new'}, 'shopify', NOW(),
              ${JSON.stringify({ source: 'shopify', email, phone, address, importRegion })}
            )
          `;
          createdCount++;
        }
        ...
        failCount++;
        if (failCount <= 10) console.error(`[${index + 1}] 处理失败: ${e.message?.slice(0, 80)}`);
      }
    });
  ...
  console.log(`📊 数据库中总记录数: ${total[0].count}`);
  process.exit(0);
}
