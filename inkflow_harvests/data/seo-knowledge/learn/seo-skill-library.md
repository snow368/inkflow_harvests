...
> **性质**：方法论层（"怎么做 SEO"）——与 `sources/`（practitioner 一手情报）、`apply/`（落地产出）并列，是 harvests SEO KB 的第三根支柱。
> **全量来源**：`D:/harvests-ai-core/scripts/seed-seo.mjs`（生成 `seo_seed.sql` → Cloudflare D1 `harvests-ai-core-db`，经 `SeoSkillLibrary.tsx` 拉取 `/harvests/seo/playbooks` 渲染到 harvests.pages.dev）。
> **收口决策（2026-07-19）**：所有 SEO 知识收口 harvests。本文件为 **harvests 单一真相源（全量）**，AI Core / D1 仅作运行态镜像。后续以本文件为准，改 KB 改这里。
...
| 知识维度 sections | 6 |
| 知识模块 chapters | 62 |
| 知识点 items（即"几百条"主体） | 316 |
| 操作技能 subskills | 25 |
| 技能目录 skills（看板） | 25 |
| 子系统 slots subsystems | 25 |
| 看板项目 projects | 1 |

> 合计知识点（items）**316** 条 + 技能 25 个，即用户记忆中的"几百条 SEO 技能知识库"。

---

## SEO 战略与排名体系 `seo-strategy`

> 150个策略速查 · 排名因子 · 竞品计分卡 · 算法应对 · 实战案例
...
- **如何找 SEO 竞争对手**
  - SEO竞品 ≠ 商业竞品：抢同关键词排名就算，不一定是同行
  - 找法1：搜5-10个核心词→记录首页前10→取重复出现的网站
  - 找法2：工具筛选（Ahrefs/SEMrush）+ Chrome插件（月访问>100K、SEO占比>40%）
  - 找法3：扩词法—核心词根→best类/alternatives类/use case类
  - 评估维度：Top Pages非品牌词内容策略 + 排名上升期外链策略
  - 来源: @CoderJeffLee 子木聊AI增长

### 操作技能（3）

- `seo-site-audit` **网站 SEO 审计** — 29项逐页检查，输出整改清单 （触发：*"审计网站SEO"*）
- `seo-competitor-gap` **竞品差距分析** — 页面/关键词/内容/技术四维对比，找排名差距 （触发：*"分析竞品"*）
- `seo-competitor-analysis` **竞品深度分析** — 页面结构/关键词覆盖/内容策略/技术指标全面对比 （触发：*"竞品深度分析"*）

---

## 关键词研究与主题集群 `seo-keyword`

> 5维评分 · 意图判断 · 长尾扩展 · 分组聚合 · 主题集群 · 月度审计
...
- **搜索意图 × 页面类型匹配（完整映射表）**
  - 【商业意图 Commercial】用户在做购买决策 → 对比/Best-of/替代品/测评页
  -   · 'best X' → Best-of榜单页 (例: best tattoo booking software)
  -   · 'X vs Y' → 对比页 (例: inkflow vs booksy)
  -   · 'X alternatives' → 替代品页 (例: booksy alternatives)
  -   · 'X review' → 测评页 (例: vagaro review)
  -   · 'X pricing' → 定价/价格对比页 (例: inkflow pricing)
  - 【交易意图 Transactional】用户想用工具/买东西 → 功能页/产品页/工具页/购买页
  -   · 'X software/app/tool' → 功能页/产品页 (例: tattoo booking app)
  -   · 'X generator' → 工具页 (例: tattoo design generator)
  -   · 'X for Y' → 功能页/场景页 (例: booking app for artists)
  -   · 'buy X / X free trial' → 定价页/注册页 (例: inkflow free trial)
  - 【信息意图 Informational】用户想学知识 → 博客/教程/指南/FAQ
  -   · 'how to X' → 教程 (例: how to start a tattoo studio)
  -   · 'what is X' → 科普文章 (例: what is a digital waiver)
  -   · 'X guide' → 完整指南 (例: tattoo aftercare guide)
  -   · 'X meaning' → 含义解读页 (例: lotus flower tattoo meaning)
  -   · 'X vs Y information' → 信息对比 (例: laser vs tattoo removal cream)
  - 【导航意图 Navigational】用户找特定品牌 → 品牌页/登录页/首页
  -   · 'X login / X app' → 登录/下载页
  -   · 'X brand name' → 首页/品牌介绍页
  - 核心公式: 搜索意图 = 用户想完成的任务 + Google用什么页面类型满足
  - 关键词优先级: 零点击时代 交易/商业 > 信息 > 导航
  - 注意: 一个关键词可能跨意图 → 需SERP拆解确认主意图 → 一页只做一个intent
  - 来源: @CoderJeffLee 子木聊AI增长

### 操作技能（4）

- `seo-keyword-finder` **关键词研究与建议** — 找词来源（GSC/AITDK/Google Suggest/Reddit）+5维评分选词+意图判断+分配到页面类型。输出：关键词→页面类型映射表 （触发：*"找关键词"*）
- `seo-topic-cluster` **主题集群规划** — 支柱页选型→集群拆分→内链双向→长尾词分配到H2/H3→权重通道 （触发：*"规划主题集群"*）
- `seo-serp-analysis` **SERP 竞争分析** — 逐条拆解首页10条结果找表层Gap+用户真实需求信息增益，输出页面策略 （触发：*"分析SERP"*）
- `seo-rank-tracker` **排名追踪** — 目标关键词排名变化监控、高曝光低点击优化、11-20名冲刺 （触发：*"追踪排名"*）

---

## 内容创作与 GEO/AEO 写作 `seo-content`

> 内容矩阵 · AEO优化 · BLUF写作法 · 案例研究 · AI内容引擎

### 知识模块（9）

- **内容矩阵（9种页面类型×格式×Schema）**
...
- **SEO 文章流水线（maks6361）**
  - ① Research: DataForSEO搜搜索量/CPC/难度
  - ② 竞品分析: 抓首页前5→找「都做了什么」+「都没做什么」两层gap
  - ③ Topic Research: 官方文档/研究报告/公司博客等一手源，不做二手堆叠
  - ④ 写文章: 按CORE-EEAT清单（intent对齐/首段150字给答案/覆盖≥3查询变体）
  - ⑤ 配图: 生成后转webp
  - ⑥ SEO自评分: 目标≥8/10
  - ⑦ 事实核查: 每500字≥1外部链接，逐一验证原始资料
  - ⑧ AI语调人性化: 消除10+种常见AI写作模式
  - ⑨ 发布: frontmatter markdown推git + 提交sitemap + Request Indexing
  - 核心: Google不care谁写的（AI或人），care有没有EEAT信号
  - 来源: @maks6361

### 操作技能（6）

- `seo-content-brief` **内容大纲生成** — 按页面类型生成完整Content Brief（标题/大纲/FAQ/内链/Schema） （触发：*"写内容大纲"*）
- `seo-content-writer` **SEO 内容写作** — 按Brief+AEO标准写正文（BLUF首段/段落自包含/列表表格优先/FAQ结构化） （触发：*"写SEO文章"*）
- `seo-content-quality` **内容质量审核** — 评估AI引用潜力：信息增量/BLUF结构/EEAT信号/FAQ真实来源/反AI腔 （触发：*"审核内容质量"*）
- `seo-meta-optimizer` **Meta标签优化** — 批量检查优化Title/Desc/OG/Twitter Card，含写作规则 （触发：*"优化meta"*）
- `seo-content-rewrite` **内容重写优化** — 按SERP gap + AEO/GEO标准诊断重写旧页面 （触发：*"重写内容"*）
- `seo-content-refresher` **内容刷新** — 已排名页面数据/年份/链接轻度更新，维持排名位置 （触发：*"刷新内容"*）

---

## 技术 SEO 规范 `seo-technical`

> URL/Schema/Meta · 内链规则 · CWV优化 · JS SEO · 多语言 · 工具库
...
- **2026 SEO 核心因子（foley_seo）**
  - URL可访问性: 爬虫/LLM能否访问、内链是否充足、导航是否无需JS、robots HTTP 200
  - URL渲染: 原始DOM vs JS渲染DOM差异、SSR vs CSR、GSC URL检查验证
  - 用户行为信号: Navboost(C-R-A-P-S)、good click vs bad click、POGO率、品牌搜量
  - 内容质量: intent对齐/EEAT/YMYL/新鲜度/NLP主题覆盖/来源引用/独特性/多媒体
  - 域名聚合质量: PageRank分布、信任指标聚合（品牌/链接/评价/搜索/情感）
  - 无效内容削减: 减少已发现未索引、淘汰无价值页、合重复URL、优化crawl budget
  - 站点行为模式: 内容发布模式一致性、新链接流入规律、内容质量突变检测
  - 来源: @foley_seo

### 操作技能（8）

- `seo-technical-check` **技术SEO检查** — 32项基础设施逐项诊断（robots/sitemap/重定向/Canonical/Hreflang/JS SEO/爬取） （触发：*"技术SEO检查"*）
- `seo-sitemap-config` **Sitemap/Robots配置** — 生成sitemap+robots+IndexNow，提高爬虫发现效率 （触发：*"配置sitemap"*）
- `seo-schema-injector` **Schema 注入** — 按页面类型注入JSON-LD结构化数据（Article/FAQ/SoftwareApplication/BreadcrumbList/ImageObject） （触发：*"加schema"*）
- `seo-speed-optimizer` **页面速度优化** — CWV达标优化（LCP/INP/CLS）+ 图片/JS/CSS/字体降耗 （触发：*"提升速度"*）
- `seo-internal-linking` **内链优化** — 内链策略/锚文本/权重分配/主题聚类，每页≥3条非nav内链 （触发：*"优化内链"*）
- `seo-entity-optimizer` **实体优化** — 识别关键实体并强化其在内容中的主题实体信号 （触发：*"优化实体"*）
- `seo-geo-optimizer` **GEO/AEO 优化** — AI Overview适配+结构化答案+FAQ被AI引用机会优化 （触发：*"优化GEO"*）
- `seo-alert-monitor` **SEO 监控告警** — 排名/流量/异常变化自动监控与通知 （触发：*"设置监控"*）

---

## 外链建设全攻略 `seo-link`

> 25种策略 · 质量评估 · B2B/B2C差异化 · Niche资源 · Webinar策略
...
- **🚨 低质换链目录反模式（Jakob/geoly 案例）**
  - 现象: 免费 do-follow 换链目录(如 geoly.net) — 注册加站、链回免费do-follow、否则$3
  - 风险: 邻居含 escort/spam 站(auto-approve无审核)=link farm→Google unnatural links惩罚
  - 结论: InkFlow(B2B SaaS)不适用 — 品牌安全+惩罚风险>收益；仅作行业认知，不提交
  - 正道: 走质量铁律(域名多样性>数量/相关性/自然度)，用B2B第一梯队+纹身Niche资源
...
- `seo-backlink-audit` **外链审计与策略** — 25种策略+分阶段执行计划 （触发：*"外链审计"*）
- `seo-outreach-writer` **Outreach 邮件** — 8种场景邮件模板+跟进节奏 （触发：*"写外链邮件"*）

---

## SEO 工作流与增长执行 `seo-workflow`

> 建站流程 · 增长策略 · CRO转化 · 社媒分发 · 出海 · GSC复盘
...
- `seo-launch-checklist` **新站上线清单** — 9步上线流程逐项验证 （触发：*"网站上线"*）
- `seo-gsc-analyzer` **GSC 数据分析** — 关键词/页面/CTR四象限分析 （触发：*"分析GSC"*）

---

## 附：SEO 进度看板数据（seo_board 镜像）

### 看板项目 projects（1）

- `ink-flows.com` InkFlow Marketing（SaaS）
...
- `seo-competitor-gap` **竞品差距分析** — 页面/关键词/内容/技术四维对比，找排名差距 （触发：*"分析竞品"*）
- `seo-competitor-analysis` **竞品深度分析** — 页面结构/关键词覆盖/内容策略/技术指标全面对比 （触发：*"竞品深度分析"*）
- `seo-keyword-finder` **关键词研究与建议** — 找词来源（GSC/AITDK/Google Suggest/Reddit）+5维评分选词+意图判断+分配到页面类型 （触发：*"找关键词"*）
- `seo-topic-cluster` **主题集群规划** — 支柱页选型→集群拆分→内链双向→长尾词分配到H2/H3→权重通道 （触发：*"规划主题集群"*）
- `seo-serp-analysis` **SERP 竞争分析** — 逐条拆解首页10条结果找表层Gap+用户真实需求信息增益，输出页面策略 （触发：*"分析SERP"*）
- `seo-rank-tracker` **排名追踪** — 目标关键词排名变化监控、高曝光低点击优化、11-20名冲刺 （触发：*"追踪排名"*）
- `seo-content-brief` **内容大纲生成** — 按页面类型生成完整Content Brief（标题/大纲/FAQ/内链/Schema） （触发：*"写内容大纲"*）
...
- `seo-geo-optimizer` **GEO/AEO 优化** — AI Overview适配+结构化答案+FAQ被AI引用机会优化 （触发：*"优化GEO"*）
- `seo-alert-monitor` **SEO 监控告警** — 排名/流量/异常变化自动监控与通知 （触发：*"设置监控"*）
- `seo-backlink-audit` **外链审计与策略** — 25种策略+分阶段执行计划+免费目录提交 （触发：*"外链审计"*）
- `seo-outreach-writer` **Outreach 邮件** — 8种场景邮件模板+跟进节奏 （触发：*"写外链邮件"*）
- `seo-launch-checklist` **新站上线清单** — 9步上线流程逐项验证（域名/HTTPS/GSC/Sitemap/Schema/CWV） （触发：*"网站上线"*）
- `seo-gsc-analyzer` **GSC 数据分析** — 关键词/页面/CTR四象限分析+Experiment Log （触发：*"分析GSC"*）

### 子系统 slots subsystems（25）

- `site-audit` 站内SEO审计 → skill `undefined`（strategy）
- `competitor-gap` 竞品差距 → skill `undefined`（strategy）
- `competitor-deep` 竞品深度分析 → skill `undefined`（strategy）
- `keyword-find` 关键词研究 → skill `undefined`（keywords）
- `topic-cluster` 主题集群 → skill `undefined`（keywords）
- `serp-analysis` SERP分析 → skill `undefined`（keywords）
- `rank-track` 排名追踪 → skill `undefined`（keywords）
- `content-brief` 内容大纲 → skill `undefined`（content）
- `content-write` SEO内容写作 → skill `undefined`（content）
- `content-quality` 内容质量审核 → skill `undefined`（content）
- `meta-optimize` Meta标签 → skill `undefined`（content）
- `content-rewrite` 内容重写 → skill `undefined`（content）
- `content-refresh` 内容刷新 → skill `undefined`（content）
- `tech-check` 技术SEO检查 → skill `undefined`（technical）
- `sitemap` Sitemap配置 → skill `undefined`（technical）
- `schema` Schema注入 → skill `undefined`（technical）
- `speed` 速度优化 → skill `undefined`（technical）
- `internal-link` 内链优化 → skill `undefined`（technical）
- `entity` 实体优化 → skill `undefined`（technical）
- `geo-aeo` GEO/AEO优化 → skill `undefined`（technical）
- `alert` 监控告警 → skill `undefined`（technical）
- `backlink` 外链审计 → skill `undefined`（links）
- `outreach` Outreach邮件 → skill `undefined`（links）
- `launch` 上线清单 → skill `undefined`（workflow）
- `gsc` GSC分析 → skill `undefined`（workflow）
