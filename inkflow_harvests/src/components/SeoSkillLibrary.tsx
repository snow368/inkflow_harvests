...
const SKILL_DATA: SkillSection[] = [
  {
    ...
    subSkills: [
      ...
      { id: 'seo-competitor-gap', name: '竞品差距分析', desc: '页面/关键词/内容/技术四维对比，找排名差距', trigger: '"分析竞品"' },
      { id: 'seo-competitor-analysis', name: '竞品深度分析', desc: '页面结构/关键词覆盖/内容策略/技术指标全面对比', trigger: '"竞品深度分析"' },
    ],
  },
  {
    id: 'seo-keyword',
    ...
    subSkills: [
      { id: 'seo-keyword-finder', name: '关键词研究与建议', desc: '找词来源（GSC/AITDK/Google Suggest/Reddit）+5维评分选词+意图判断+分配到页面类型。输出：关键词→页面类型映射表', trigger: '"找关键词"' },
      { id: 'seo-topic-cluster', name: '主题集群规划', desc: '支柱页选型→集群拆分→内链双向→长尾词分配到H2/H3→权重通道', trigger: '"规划主题集群"' },
      { id: 'seo-serp-analysis', name: 'SERP 竞争分析', desc: '逐条拆解首页10条结果找表层Gap+用户真实需求信息增益，输出页面策略', trigger: '"分析SERP"' },
      { id: 'seo-rank-tracker', name: '排名追踪', desc: '目标关键词排名变化监控、高曝光低点击优化、11-20名冲刺', trigger: '"追踪排名"' },
    ],
  },
  {
    id: 'seo-content',
    ...
    chapters: [
      ...
      { label: 'SEO 文章流水线（maks6361）', items: [
    ...
    subSkills: [
      { id: 'seo-content-brief', name: '内容大纲生成', desc: '按页面类型生成完整Content Brief（标题/大纲/FAQ/内链/Schema）', trigger: '"写内容大纲"' },
      { id: 'seo-content-writer', name: 'SEO 内容写作', desc: '按Brief+AEO标准写正文（BLUF首段/段落自包含/列表表格优先/FAQ结构化）', trigger: '"写SEO文章"' },
      { id: 'seo-content-quality', name: '内容质量审核', desc: '评估AI引用潜力：信息增量/BLUF结构/EEAT信号/FAQ真实来源/反AI腔', trigger: '"审核内容质量"' },
      { id: 'seo-meta-optimizer', name: 'Meta标签优化', desc: '批量检查优化Title/Desc/OG/Twitter Card，含写作规则', trigger: '"优化meta"' },
      { id: 'seo-content-rewrite', name: '内容重写优化', desc: '按SERP gap + AEO/GEO标准诊断重写旧页面', trigger: '"重写内容"' },
      { id: 'seo-content-refresher', name: '内容刷新', desc: '已排名页面数据/年份/链接轻度更新，维持排名位置', trigger: '"刷新内容"' },
    ],
  },
  {
    id: 'seo-technical',
    ...
    chapters: [
      ...
      { label: '2026 SEO 核心因子（foley_seo）', items: [
    ...
    subSkills: [
      { id: 'seo-technical-check', name: '技术SEO检查', desc: '32项基础设施逐项诊断（robots/sitemap/重定向/Canonical/Hreflang/JS SEO/爬取）', trigger: '"技术SEO检查"' },
      { id: 'seo-sitemap-config', name: 'Sitemap/Robots配置', desc: '生成sitemap+robots+IndexNow，提高爬虫发现效率', trigger: '"配置sitemap"' },
      { id: 'seo-schema-injector', name: 'Schema 注入', desc: '按页面类型注入JSON-LD结构化数据（Article/FAQ/SoftwareApplication/BreadcrumbList/ImageObject）', trigger: '"加schema"' },
      { id: 'seo-speed-optimizer', name: '页面速度优化', desc: 'CWV达标优化（LCP/INP/CLS）+ 图片/JS/CSS/字体降耗', trigger: '"提升速度"' },
      { id: 'seo-internal-linking', name: '内链优化', desc: '内链策略/锚文本/权重分配/主题聚类，每页≥3条非nav内链', trigger: '"优化内链"' },
      { id: 'seo-entity-optimizer', name: '实体优化', desc: '识别关键实体并强化其在内容中的主题实体信号', trigger: '"优化实体"' },
      { id: 'seo-geo-optimizer', name: 'GEO/AEO 优化', desc: 'AI Overview适配+结构化答案+FAQ被AI引用机会优化', trigger: '"优化GEO"' },
      { id: 'seo-alert-monitor', name: 'SEO 监控告警', desc: '排名/流量/异常变化自动监控与通知', trigger: '"设置监控"' },
    ],
  },
  {
    id: 'seo-link',
    ...
    chapters: [
      ...
      { label: '🚨 低质换链目录反模式（Jakob/geoly 案例）', items: [
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="text-center text-[10px] text-slate-600 pt-2">
        ...
        <br />
        6 个知识型技能 + 25 个操作型技能 · 新知识入库后自动拆入对应技能维度
      </div>
    </div>
  );
}
