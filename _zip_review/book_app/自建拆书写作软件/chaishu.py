...
class SimpleLLM:
    ...
    def chat(self, prompt, retry_count=3):
        ...
        route = "relay" if use_relay else "direct"
        self._log(f"LLM route={route} model={self.model} url={url}")

        token = self.relay_token if (use_relay and self.relay_token) else self.api_key
        headers = {
        ...
        for attempt in range(1, retry_count + 1):
            try:
                self._log(f"LLM request attempt={attempt}/{retry_count}")
                resp = self.session.post(url, headers=headers, json=data, timeout=(10, 120))
                if resp.status_code == 200:
                    self._log("LLM response status=200 success")
                    return resp.json()["choices"][0]["message"]["content"]
                error_msg = f"HTTP {resp.status_code}: {resp.text[:200]}"
                self._log(f"LLM response failed {error_msg}")
                last_exception = Exception(error_msg)
                if attempt == retry_count:
                    return f"错误：{error_msg}"
                time.sleep(1 * attempt)
            except requests.exceptions.Timeout:
                self._log("LLM timeout")
                last_exception = Exception("请求超时")
                if attempt == retry_count:
                    return "请求异常：超时"
                time.sleep(1 * attempt)
            except requests.exceptions.ConnectionError as e:
                self._log(f"LLM connection error: {str(e)}")
                last_exception = e
                if attempt == retry_count:
                    return f"请求异常：连接错误 - {str(e)}"
                time.sleep(1 * attempt)
            except Exception as e:
                self._log(f"LLM unexpected error: {str(e)}")
                last_exception = e
                if attempt == retry_count:
                    return f"请求异常：{str(e)}"
                time.sleep(1 * attempt)
...
class SimpleWorkbench:
    MODEL_PRESETS = {
        ...
        "claude-openrouter-sonnet": ("Claude Sonnet (OpenRouter)", "https://openrouter.ai/api/v1", "anthropic/claude-3.7-sonnet"),
        "claude-openrouter-opus": ("Claude Opus (OpenRouter)", "https://openrouter.ai/api/v1", "anthropic/claude-3-opus"),
        "gemini-openrouter": ("Gemini 2.5 Pro (OpenRouter)", "https://openrouter.ai/api/v1", "google/gemini-2.5-pro"),
...
"""
    ...
    def create_book_analysis_page(self):
        ...
        self.route_btn = ttk.Button(btn_frame, text="智能流程推荐", command=self.recommend_process_for_selected_book)
        ...
        route_frame = ttk.LabelFrame(left_frame, text="流程推荐（按资料动态路由）", padding=8)
        route_frame.pack(fill=tk.X, pady=5)
        self.process_route_text = scrolledtext.ScrolledText(route_frame, wrap=tk.WORD, height=8, font=('微软雅黑', 9))
        self.process_route_text.pack(fill=tk.X, expand=False)
        self.process_route_text.insert(tk.END, "选择文档后点击「智能流程推荐」，生成建议流程。")
    ...
    def _build_dynamic_process_route(self, title, content):
        text = (content or "")[:40000]
        signals = {
            "suspense": sum(text.count(k) for k in ["悬疑", "凶手", "线索", "反转", "真相", "谜", "案件"]),
            "romance": sum(text.count(k) for k in ["言情", "感情", "告白", "误会", "心动", "分手", "和好"]),
            "slice": sum(text.count(k) for k in ["家庭", "婚姻", "职场", "现实", "日常", "人情", "世情"]),
            "shortform": sum(text.count(k) for k in ["短篇", "盐言", "快节奏", "反转文", "高能"]),
        }
        main_genre = max(signals, key=signals.get) if any(signals.values()) else "general"

        if main_genre == "suspense":
            route_name = "悬疑路由"
            focus = ["线索密度", "误导/反转频率", "危险升级速度", "章末悬钩强度"]
            steps = ["先做线索图谱", "再做反转节拍表", "最后做章节级悬钩校验"]
        elif main_genre == "romance":
            route_name = "情感路由"
            focus = ["关系拉扯强度", "误会-和解结构", "情感兑现时机", "高情绪场景比例"]
            steps = ["先做关系张力图", "再做情绪峰谷分布", "最后做付费前后情绪钩子"]
        elif main_genre == "slice":
            route_name = "世情路由"
            focus = ["现实细节真实度", "共鸣点密度", "冲突生活化强度", "人物动机一致性"]
            steps = ["先做角色动机网", "再做冲突递进表", "最后做共鸣点覆盖检查"]
        elif main_genre == "shortform":
            route_name = "短篇路由"
            focus = ["开篇抓力", "信息密度", "结尾回收", "转折命中率"]
            steps = ["先测首段钩子", "再测中段推进", "最后测结尾回收与反转命中"]
        else:
            route_name = "通用路由"
            focus = ["开篇钩子", "章节留存", "节奏密度", "角色关系张力", "情绪操控"]
            steps = ["先做六维诊断", "再输出问题优先级", "最后生成修订动作清单"]
    ...
    def recommend_process_for_selected_book(self):
        ...
        route = self._build_dynamic_process_route(selected_title, content)
        lines = [
            f"文档：{route['title']}",
            f"推荐路由：{route['route_name']}",
            "",
            "题材信号：",
            f"- suspense: {route['signals']['suspense']}",
            f"- romance: {route['signals']['romance']}",
            f"- slice_of_life: {route['signals']['slice']}",
            f"- shortform: {route['signals']['shortform']}",
            "",
            "本次优先拆解维度："
        ]
        for x in route["focus"]:
            lines.append(f"- {x}")
        lines.append("")
        lines.append("建议执行顺序：")
        for i, x in enumerate(route["steps"], start=1):
            lines.append(f"{i}. {x}")

        if hasattr(self, "process_route_text"):
            self.process_route_text.delete(1.0, tk.END)
            self.process_route_text.insert(tk.END, "\n".join(lines))
...
请确保提示词具体、可操作，并且完全原创。书名和简介必须严格按照上述要求创作。"""
    ...
    def create_book_analysis(self):
        ...
        try:
            ...
            if hasattr(self, "process_route_text"):
                self.process_route_text.delete(1.0, tk.END)
                self.process_route_text.insert(
                    tk.END,
                    f"文档：{selected_title}\n推荐路由：{route['route_name']}\n优先维度：{' / '.join(route['focus'])}\n"
                )
        except Exception:
            pass
...
"""
                ...
                if hasattr(self, "route_btn"):
                    self.root.after(0, lambda: self.route_btn.config(state=tk.NORMAL))
...
要求：语言流畅，描写生动，直接输出章节正文。章节开头格式为"第{ch}章 {ch_title}" """
    ...
    def generate_chapters_by_list_with_caogao(self, chapter_list, caogao_dir):
        if not chapter_list:
            return
        self.log_to_write(f"Start generating chapter drafts to caogao folder: {chapter_list}")
        self.progress_var.set(0)
        self.progress_label_var.set("Generating chapter drafts...")
        ...
        def task():
            ...
            for idx, ch in enumerate(chapter_list):
                bp = self.chapter_blueprints.get(ch, "")
                if not bp:
                    self.log_to_write(f"Chapter {ch} blueprint missing, skip")
                    continue

                ch_title = self.extract_chapter_title(bp)
                self.log_to_write(f"Generating Chapter {ch} - {ch_title}...")

                prompt = f"""You are a novelist. Write a full chapter based on this blueprint (about {self.words_per_chapter.get()} words).
...
Requirements:
1. Keep narrative continuity.
2. Follow the blueprint; avoid filler.
3. End with a hook.

Output format: Chapter {ch} {ch_title}
Output only chapter text."""

                draft = llm.chat(prompt)
                if draft.startswith("Error") or draft.startswith("Request Exception"):
                    self.log_to_write(f"Chapter {ch} generation failed: {draft}")
                    continue
                ...
                try:
                    ...
                    self.log_to_write(f"Chapter {ch} saved: {filepath}")
                except Exception as e:
                    self.log_to_write(f"Chapter {ch} save failed: {e}")

                progress = int((idx + 1) / total * 100)
                self.root.after(0, lambda p=progress: self.progress_var.set(p))

            self.root.after(0, self.merge_all_chapters_to_full_document)
            self.log_to_write("Draft generation completed and merged")
            self.root.after(0, lambda: self.progress_label_var.set("Draft generation completed"))
    ...
    def create_system_settings_page(self):
        ...
        row += 1
        ttk.Label(config_frame, text="连接日志:").grid(row=row, column=0, sticky='nw', padx=5, pady=5)
        self.api_log_text = scrolledtext.ScrolledText(config_frame, wrap=tk.WORD, height=8, width=80, font=('Consolas', 9))
        self.api_log_text.grid(row=row, column=1, columnspan=2, sticky='w', padx=5, pady=5)
        row += 1
        ttk.Button(config_frame, text="清空连接日志", command=lambda: self.api_log_text.delete(1.0, tk.END)).grid(row=row, column=1, sticky='w', padx=5, pady=2)
        proxy_frame = ttk.LabelFrame(page, text="代理设置", padding=10)
        proxy_frame.pack(fill=tk.X, padx=20, pady=10)
        ttk.Checkbutton(proxy_frame, text="启用中转站（Relay Gateway）", variable=self.relay_enabled).grid(row=0, column=0, columnspan=2, sticky='w', padx=5, pady=5)
        ttk.Label(proxy_frame, text="中转URL:").grid(row=1, column=0, sticky='w', padx=5, pady=5)
        ttk.Entry(proxy_frame, textvariable=self.relay_url, width=60).grid(row=1, column=1, sticky='w', padx=5, pady=5)
        ttk.Label(proxy_frame, text="中转Token:").grid(row=2, column=0, sticky='w', padx=5, pady=5)
        ttk.Entry(proxy_frame, textvariable=self.relay_token, width=60, show="*").grid(row=2, column=1, sticky='w', padx=5, pady=5)
        ttk.Label(proxy_frame, text="中转Provider(可选):").grid(row=3, column=0, sticky='w', padx=5, pady=5)
        ttk.Entry(proxy_frame, textvariable=self.relay_provider, width=30).grid(row=3, column=1, sticky='w', padx=5, pady=5)
        ttk.Label(proxy_frame, text="开启后，请求会发到中转URL，并附带上游 base_url/api_key/model。", foreground="#666666").grid(row=4, column=0, columnspan=2, sticky='w', padx=5, pady=2)
    ...
    def test_api(self):
        ...
        def test():
            try:
                llm = self.get_llm()
                result = llm.chat("请回复：OK", retry_count=1)
                if "错误" in result or "异常" in result:
                    msg = f"[{datetime.now().strftime('%H:%M:%S')}] ❌ API测试失败: {result}"
                else:
                    route = "relay" if (self.relay_enabled.get() and self.relay_url.get().strip()) else "direct"
                    msg = f"[{datetime.now().strftime('%H:%M:%S')}] ✅ API测试成功（{route}）"
            except Exception as e:
    ...
    def log_to_system(self, msg):
        ...
        if hasattr(self, 'test_log_text'):
            self.test_log_text.insert(tk.END, ts_msg + "\n")
            self.test_log_text.see(tk.END)
        if hasattr(self, 'api_log_text'):
            self.api_log_text.insert(tk.END, ts_msg + "\n")
