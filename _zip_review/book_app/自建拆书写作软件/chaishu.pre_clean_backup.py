...
要求：语言流畅，描写生动，直接输出章节正文。章节开头格式为"第{ch}章 {ch_title}" """
    ...
    def generate_chapters_by_list_with_caogao(self, chapter_list, caogao_dir):
        if not chapter_list:
            return
        self.log_to_write(f"????????? caogao ??: {chapter_list}")
        self.progress_var.set(0)
        ...
        def task():
            ...
            for idx, ch in enumerate(chapter_list):
                bp = self.chapter_blueprints.get(ch, "")
                if not bp:
                    self.log_to_write(f"?{ch}????????")
                    continue

                ch_title = self.extract_chapter_title(bp)
                self.log_to_write(f"?????{ch}??{ch_title}?...")

                prompt = f"""???????????????????????????{self.words_per_chapter.get()}???
...
??????"""
                ...
                try:
                    with open(filepath, 'w', encoding='utf-8') as f:
                        f.write(f"?{ch}? {ch_title}\n\n{draft}")
                    self.log_to_write(f"?{ch}????: {filepath}")
                except Exception as e:
                    self.log_to_write(f"?{ch}?????: {e}")
