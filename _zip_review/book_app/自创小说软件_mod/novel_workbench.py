...
class NovelWorkbench:
    ...
    def __init__(self):
        ...
        self.quality_threshold = tk.DoubleVar(value=7.8)
        self.max_rewrite_rounds = tk.IntVar(value=1)
    ...
    def save_project_state(self):
        ...
        state = {
            ...
            'chapter_scores': self.chapter_scores,
    ...
    def load_project_state(self):
        ...
        try:
            ...
            self.chapter_scores = {int(k): v for k, v in state.get('chapter_scores', {}).items()}
    ...
    def create_creation_center(self):
        ...
        ttk.Button(step_frame, text="Step3+: Draft + Score + Rewrite", command=self.gen_all_drafts_with_qc).pack(fill=tk.X, pady=2)
        ...
        qc_cfg = ttk.Frame(step_frame)
        qc_cfg.pack(fill=tk.X, pady=3)
        ttk.Label(qc_cfg, text="Score Threshold").pack(side=tk.LEFT)
        tk.Spinbox(qc_cfg, from_=5.0, to=9.5, increment=0.1, textvariable=self.quality_threshold, width=5).pack(side=tk.LEFT, padx=4)
        ttk.Label(qc_cfg, text="Rewrite Rounds").pack(side=tk.LEFT, padx=(8, 0))
        tk.Spinbox(qc_cfg, from_=0, to=3, textvariable=self.max_rewrite_rounds, width=4).pack(side=tk.LEFT, padx=4)
...
要求：语言流畅，描写生动，直接输出章节正文。"""
    ...
    def gen_all_drafts_with_qc(self):
        if not self.chapter_blueprints:
            messagebox.showwarning("Notice", "Please run Step2 first to generate chapter blueprints.")
            return
        self.gen_drafts_range_with_qc(1, self.chapter_num.get())
    ...
    def evaluate_chapter_draft(self, llm, ch, bp, draft):
        prompt = f"""You are a strict fiction editor.
Evaluate Chapter {ch} and return ONLY valid JSON:
{{
  "total_score": 0-10,
  "scores": {{
    "hook": 0-10,
  ...
  "weak_points": ["at least 3 specific weaknesses"],
  "rewrite_actions": ["at least 3 concrete rewrite actions"]
}}
...
"""
        ...
        return {
            "total_score": 6.0,
            "scores": {"hook": 6, "conflict": 6, "pacing": 6, "emotion": 6, "ending_hook": 6},
            "weak_points": ["Evaluation parser fallback was used."],
            "rewrite_actions": ["Strengthen opening hook", "Increase conflict intensity", "Tighten paragraph pacing"]
        }

    def rewrite_chapter_draft(self, llm, ch, bp, draft, eval_data):
        weak = eval_data.get("weak_points", [])
        actions = eval_data.get("rewrite_actions", [])
        weak_txt = "\n".join([f"- {x}" for x in weak])
        action_txt = "\n".join([f"- {x}" for x in actions])
        prompt = f"""You are rewriting Chapter {ch}.
Keep the core plot and character consistency, but improve quality using the editor notes.
...
Requirements:
1) Return full chapter text only (no explanation).
2) Keep original style while improving readability, hook, tension, and retention.
"""
        return llm.chat(prompt)

    def gen_drafts_range_with_qc(self, start, end):
        if not self.chapter_blueprints:
            messagebox.showwarning("Notice", "Please run Step2 first to generate chapter blueprints.")
            return
        total = self.chapter_num.get()
        ...
        threshold = float(self.quality_threshold.get())
        max_rounds = int(self.max_rewrite_rounds.get())
        self.log(f"Start one-pass draft+QC for chapters {start}-{end}, threshold={threshold} ...")
        ...
        def task():
            ...
            for ch in range(start, end + 1):
                bp = self.chapter_blueprints.get(ch, "")
                if not bp:
                    self.log(f"Chapter {ch}: blueprint missing, skipped.")
                    continue
                if ch in self.chapter_drafts:
                    self.log(f"Chapter {ch}: draft already exists, skipped.")
                    continue

                self.log(f"Chapter {ch}: generating initial draft ...")
                prompt = f"""You are a novelist. Write a complete chapter (~{self.words_per_chapter.get()} words) based on this blueprint.
Blueprint:
{bp}

Output chapter text only."""
                best_draft = llm.chat(prompt)
                best_eval = self.evaluate_chapter_draft(llm, ch, bp, best_draft)
                best_score = float(best_eval.get("total_score", 0) or 0)
                self.log(f"Chapter {ch}: initial score {best_score:.1f}/10")

                rounds = 0
                while best_score < threshold and rounds < max_rounds:
                    rounds += 1
                    self.log(f"Chapter {ch}: rewrite round {rounds} ...")
                    rewritten = self.rewrite_chapter_draft(llm, ch, bp, best_draft, best_eval)
                    re_eval = self.evaluate_chapter_draft(llm, ch, bp, rewritten)
                    re_score = float(re_eval.get("total_score", 0) or 0)
                    self.log(f"Chapter {ch}: round {rounds} score {re_score:.1f}/10")
                    if re_score >= best_score:
                        best_draft, best_eval, best_score = rewritten, re_eval, re_score

                self.chapter_drafts[ch] = best_draft
                self.chapter_scores[ch] = best_eval
                self.text_draft.insert(tk.END, f"\n========== Chapter {ch} | Final Score {best_score:.1f}/10 ==========\n{best_draft}\n\n")
                self.text_draft.see(tk.END)

            self.log(f"Completed one-pass draft+QC for chapters {start}-{end}.")
